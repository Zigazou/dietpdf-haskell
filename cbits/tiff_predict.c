...
void tiff_predict_row_2bits_1component(uint8_t *row, size_t size,
                                       uint32_t width) {
  ...
  for (int64_t i = size - 1; i >= 0; i--) {
    current_pixel0 = GXX______(row[i]);
    current_pixel1 = G__XX____(row[i]);
    current_pixel2 = G____XX__(row[i]);
    current_pixel3 = G______XX(row[i]);

    previous_pixel0 = i >= 1 ? G______XX(row[i - 1]) : 0;

    row[i] = (uint8_t)(SXX______(current_pixel0 - previous_pixel0) |
                       S__XX____(current_pixel1 - current_pixel0) |
                       S____XX__(current_pixel2 - current_pixel1) |
                       S______XX(current_pixel3 - current_pixel2));
  }
...
void tiff_predict_row_2bits_2components(uint8_t *row, size_t size,
                                        uint32_t width) {
...
void tiff_predict_row_2bits_3components(uint8_t *row, size_t size,
                                        uint32_t width) {
  uint8_t current_component0 = 0, current_component1 = 0,
  ...
  for (int64_t i = size - 1; i >= 0; i--) {
    ...
    if (boundary == 0) {
      ...
      previous_component0 = i >= 2 ? G______XX(row[i - 2]) : 0;
      previous_component1 = i >= 1 ? GXX______(row[i - 1]) : 0;
      previous_component2 = i >= 1 ? G__XX____(row[i - 1]) : 0;

      current_component0 = i >= 1 ? G____XX__(row[i - 1]) : 0;
      current_component1 = i >= 1 ? G______XX(row[i - 1]) : 0;
      current_component2 = GXX______(row[i]);
      nibble = G__XXXXXX(row[i]);

      row[i] = (uint8_t)(SXX______(current_component2 - previous_component2) |
                         nibble);

      if (i > 0) {
        row[i - 1] =
            (uint8_t)(S____XX__(current_component0 - previous_component0) |
                      S______XX(current_component1 - previous_component1) |
                      SXX______(previous_component1) |
      ...
      previous_component0 = i > 0 ? GXX______(row[i - 1]) : 0;
      previous_component1 = i > 0 ? G__XX____(row[i - 1]) : 0;
      previous_component2 = i > 0 ? G____XX__(row[i - 1]) : 0;

      current_component0 = i > 0 ? G______XX(row[i - 1]) : 0;
      current_component1 = GXX______(row[i]);
      current_component2 = G__XX____(row[i]);
      nibble = G____XXXX(row[i]);

      row[i] = (uint8_t)(SXX______(current_component1 - previous_component1) |
                         S__XX____(current_component2 - previous_component2) |
                         nibble);
      if (i > 0) {
        row[i - 1] =
            (uint8_t)(SXX______(previous_component0) |
                      S__XX____(previous_component1) |
...
void tiff_predict_row_2bits_4components(uint8_t *row, size_t size,
                                        uint32_t width) {
  uint8_t current_component0 = 0, current_component1 = 0,
...
void tiff_predict_row_4bits_1component(uint8_t *row, size_t size,
                                       uint32_t width) {
...
void tiff_predict_row_4bits_2components(uint8_t *row, size_t size,
                                        uint32_t width) {
  uint8_t current_component0 = 0, current_component1 = 0;
  uint8_t previous_component0 = 0, previous_component1 = 0;
  ...
  for (int64_t i = size - 1; i >= 1; i--) {
    ...
    previous_component0 = GXXXX____(row[i - 1]);
    previous_component1 = G____XXXX(row[i - 1]);
...
void tiff_predict_row_4bits_3components(uint8_t *row, size_t size,
                                        uint32_t width) {
  uint8_t current_component0 = 0, current_component1 = 0,
          current_component2 = 0;
  uint8_t previous_component0 = 0, previous_component1 = 0,
  ...
  for (int64_t i = size - 1; i >= 0; i--) {
    ...
    if (alternate) {
      ...
      previous_component0 = i > 2 ? G____XXXX(row[i - 3]) : 0;
      previous_component1 = i > 1 ? GXXXX____(row[i - 2]) : 0;
      previous_component2 = i > 1 ? G____XXXX(row[i - 2]) : 0;

      current_component0 = i > 0 ? GXXXX____(row[i - 1]) : 0;
      current_component1 = i > 0 ? G____XXXX(row[i - 1]) : 0;
      ...
      previous_component0 = i > 1 ? GXXXX____(row[i - 2]) : 0;
      previous_component1 = i > 1 ? G____XXXX(row[i - 2]) : 0;
      previous_component2 = i > 0 ? GXXXX____(row[i - 1]) : 0;

      current_component0 = i > 0 ? G____XXXX(row[i - 1]) : 0;
      current_component1 = GXXXX____(row[i]);
      current_component2 = G____XXXX(row[i]);

      if (i > 0) {
        row[i - 1] =
            (uint8_t)(SXXXX____(previous_component2) |
                      S____XXXX(current_component0 - previous_component0));
      }

      row[i] = (uint8_t)(SXXXX____(current_component1 - previous_component1) |
...
void tiff_predict_row_4bits_4components(uint8_t *row, size_t size,
                                        uint32_t width) {
...
void tiff_predict_row_8bits(uint8_t *row, size_t size, uint32_t width,
                            uint8_t color_components) {
  size_t row_bytes = width * color_components;

  for (int64_t i = row_bytes - 1; i >= color_components; i--) {
    row[i] = (uint8_t)(row[i] - row[i - color_components]);
  }
}

void tiff_predict_row_16bits(uint8_t *row, size_t size, uint32_t width,
                             uint8_t color_components) {
  ...
  for (int64_t i = row_words - 1; i >= color_components; i--) {
    current_component = SWAP16(row16[i]);
    previous_component = SWAP16(row16[i - color_components]);

    row16[i] = SWAP16(current_component - previous_component);
  }
}
...
void tiff_predict_row_ffi(uint8_t *row, size_t size, uint32_t width,
  ...
  if (bits_per_component == 2) {
    if (color_components == 1) {
      tiff_predict_row_2bits_1component(row, size, width);
    } else if (color_components == 2) {
      tiff_predict_row_2bits_2components(row, size, width);
    } else if (color_components == 3) {
      tiff_predict_row_2bits_3components(row, size, width);
    } else if (color_components == 4) {
      tiff_predict_row_2bits_4components(row, size, width);
    }
  } else if (bits_per_component == 4) {
    if (color_components == 1) {
      tiff_predict_row_4bits_1component(row, size, width);
    } else if (color_components == 2) {
      tiff_predict_row_4bits_2components(row, size, width);
    } else if (color_components == 3) {
      tiff_predict_row_4bits_3components(row, size, width);
    } else if (color_components == 4) {
      tiff_predict_row_4bits_4components(row, size, width);
    }
  } else if (bits_per_component == 8) {
    tiff_predict_row_8bits(row, size, width, color_components);
  } else if (bits_per_component == 16) {
    tiff_predict_row_16bits(row, size, width, color_components);
  }
}
